...
class DrawingProgram
{
    ...
    static void EnterDrawingMode()
    {
        ...
        bool[] keyStates = new bool[4]; // Array to track the state of arrow keys (left, right, up, down)

        while (isDrawing)
        {
            RedrawCanvas(); // Redraw canvas to show the drawing and cursor

            // Capture continuous key input for arrow keys
            var input = Console.ReadKey(true);

            // Process continuous drawing when an arrow key is held down
            ...
            switch(input.Key)
                {
                case ConsoleKey.UpArrow: cursorY--; break;
                case ConsoleKey.DownArrow: cursorY++; break;
                    case ConsoleKey.LeftArrow: cursorX--; break;
                    case ConsoleKey.RightArrow: cursorX++; break;
            }


            if (keyStates[0] || keyStates[1] || keyStates[2] || keyStates[3])
            {
                // Redraw at the new cursor position
                Console.SetCursorPosition(cursorX, cursorY);
